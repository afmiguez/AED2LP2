...
import edu.ufp.inf.lp2.map.MapGraph;
...
public class DijkstraSP {
    ...
    private MapEdge[] edgeTo;    // edgeTo[v] = last edge on shortest s->v path
     ...
     */
    public DijkstraSP(MapGraph G, int s) {
	for(MapEdge e:G.edges()){
            if (e.weight() < 0)
                throw new IllegalArgumentException("edge " + e + " has negative weight");
        }

        distTo = new double[G.V()];
        edgeTo = new MapEdge[G.V()];
        for (int v = 0; v < G.V(); v++)
     ...
     */
    public Iterable<MapEdge> pathTo(int v) {
        if (!hasPathTo(v)) return null;
        Stack<MapEdge> path = new Stack<MapEdge>();
        for (MapEdge e = edgeTo[v]; e != null; e = edgeTo[e.from()]) {
            path.push(e);
        }
        return path;
    }
    ...
    private boolean check(MapGraph G, int s) {
